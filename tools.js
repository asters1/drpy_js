...
// 执行node命令并运行a.js文件
// let result = shell.exec('node "./debug.js"')
...
var VOD = {} // 二级的单个数据
var TABS = [] // 二级的自定义线路列表 如: TABS=['道长在线','道长在线2']
var LISTS = [] // 二级的自定义选集播放列表 如: LISTS=[['第1集$http://1.mp4','第2集$http://2.mp4'],['第3集$http://1.mp4','第4集$http://2.mp4']]
...
async function searchParse(searchObj) {
  ...
  if (p.startsWith('js:')) {
    ...
    var jsf = p.trim().replace('js:', '')
    jsf = jsf.replaceAll('request(', 'await request(')
    // var_to_str()
    var vs = {}
    vs.input = input
    vs.rule = rule
    vs.cfg = cfg
    // console.log(var_to_str(vs))

    var js_str = `import "./tools.js"\n` + var_to_str(vs) + '\n' + jsf
    js_str = js_str.replace(
      SETRESULT_REGEX,
      ' console.log(JSON.stringify($1($2))) ',
    )
    var res = await evals(js_str)
    if (cfg.print_switch) {
      console.log(res)
    }
...
async function detailParse(detailObj) {
  ...
  if (p === '*') {
    ...
    var res_d = await evals(js_str)
    if (cfg.print_switch) {
      console.log(res_d)
    }
    var res = JSON.parse(res_d)
    VOD = res
    // console.log(res)
    vod = VOD
    // console.log(JSON.stringify(vod))
...
async function playParse(playObj) {
  ...
  if (!rule.play_parse || !rule.lazy) {
    ...
    try {
      ...
      lazy_code.replaceAll('\\', '\\\\')
      var js_str =
        `import "./tools.js"\n` + var_to_str(playObj) + '\n' + lazy_code
      var res_play = await evals(js_str)
